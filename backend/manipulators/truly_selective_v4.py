"""
Truly Selective Manipulation V4
For repeated characters with different visuals:
Uses Latin Small Letter A (U+0061) for first 'a' → 'h'
Uses Latin Small Letter Alpha (U+0251) for second 'a' → 'o'
Maps similar-looking Unicode alternates to achieve different visuals
"""
...
class TrulySelectiveManipulatorV4:
    ...
    def create_manipulation(self, visual_word, hidden_word):
        """
        Create manipulation using alternate Unicode codepoints.

        Strategy: For repeated characters, use visually similar alternates:
        - a → ɑ (U+0251 Latin Small Letter Alpha)
        - e → ə (U+0259 Latin Small Letter Schwa)
